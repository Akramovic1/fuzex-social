...
- `fuzex.app` is reserved for the main app: marketing site, web app,
  booking API, AI agent, payments. Email reputation for the main app
  remains on `email.fuzex.app`.
- `fuzex.social` hosts everything atproto/social-related: PDS, fuzex-api,
  user handles, and the synthetic email domain (`email.fuzex.social`)
  used for phone-only signups that have no real email.
- This split mirrors the architectural separation: two systems, two
  domains, evolving independently.
- Production hostnames will be `api.fuzex.social`, `pds.fuzex.social`,
  `*.fuzex.social` (no env prefix on the bare domain).
- Dev hostnames are `dev-api.fuzex.social`, `pds.dev.fuzex.social`,
  `*.dev.fuzex.social` (env prefix `dev` is part of the subdomain).

## Migration lessons

- Bluesky's relay and AppView aggressively cache handle resolutions.
  When a handle's domain changes, the relay needs to re-resolve (can take
  up to ~1 hour for the cache to clear). Pre-launch is the cheapest time
  to do this kind of change.
- Caddy `*.dev.fuzex.app` wildcard had to be REMOVED entirely after
...
- **Hetzner blocks outbound SMTP ports 25, 465, and 2525 by default**; only port 587 is open. Bluesky PDS's email module hardcodes `smtps://` to mean "TLS-on-connect on 465" and silently no-ops if the URL is reconfigured to use 587 (returns `{"sent":true}` from `com.atproto.admin.sendEmail` with ~40ms response time and zero SMTP log lines — but no email is sent). Tested workarounds: `smtp://...:587` and `smtps://...:587` both fail to deliver from PDS, even though direct `swaks --tls --port 587` from the same VPS works perfectly. Open a Hetzner ticket to unblock 465, OR bypass PDS entirely and call Resend HTTP API from fuzex-api when email is actually needed (more reliable; uses port 443 which is never blocked). For dev, PDS email is not required for migration verification or mobile integration. x1
- **PDS sqlite orphans from failed createAccount**: when fuzex-api's `POST /v1/atproto/createAccount` succeeds at the PDS step but fails downstream (e.g., DB insert, Firestore validation, PDS admin auth 401), the PDS account row in `/pds/account.sqlite` is created but the matching Postgres `users` row is never written. The orphan blocks future signups using the same email (UNIQUE constraint on `account_email_lower_idx`). Cleanup: `sqlite3 /pds/account.sqlite "DELETE FROM account WHERE did='did:plc:xxx'; DELETE FROM actor WHERE did='did:plc:xxx';"`. Detection: `sqlite3 /pds/account.sqlite "SELECT did, email FROM account;"` should match `SELECT did, username FROM users;` in Postgres. Mismatches indicate orphans. x1
...
- Firestore is the source of truth for user-input profile data (`name`, `dateOfBirth`, `gender`, `walletAddress`). The mobile-facing field is `name` in Firestore — fuzex-api maps it internally to the `display_name` Postgres column and to atproto's `displayName` profile field. fuzex-api reads it from Firestore at signup time and copies relevant fields into Postgres for atproto-related queries. There's intentional duplication between Firestore and Postgres because they serve different access patterns (Firestore = real-time mobile reads, Postgres = atproto handle resolution from Caddy/Bluesky).

- For phone-only signups (no email in Firebase token), fuzex-api synthesizes a placeholder email of the form `phone-{e164-digits-only}@email.fuzex.social` to satisfy PDS's `createAccount` email requirement. The user never sees this email; PDS uses our auto-generated password (not user-facing) for any internal needs.
